"""NPB DugoutData 빌더 — extract → transform → team build → DugoutData."""
...
from .transform import calculate_npb_league_stats, convert_batter, convert_pitcher
...
def build_npb_data(
    ...
    """NPB 시즌 데이터를 로드하여 DugoutData로 반환."""
    engine_cache = Path(cache_dir) / "engine" / f"npb_data_{season}.pkl"

    if not force_refresh and engine_cache.exists():
        logger.info("Loading cached NPB engine data: %s", engine_cache)
    ...
    # 2. League stats
    league = calculate_npb_league_stats(batters_raw, season)
    logger.info(
        "NPB league stats: K%%=%.3f, BB%%=%.3f, HR/BIP=%.3f",
    ...
    logger.info("NPB: %d batters, %d pitchers converted", len(all_batters), len(all_pitchers))
    ...
    for team_id, team_info in TEAM_MAPPING.items():
        ...
        if not team_batters or not team_pitchers_dict:
            logger.warning("NPB team %s has no data, skipping", team_id)
            continue
    ...
    logger.info("NPB: %d teams built", len(teams))
    ...
    with open(engine_cache, "wb") as f:
        pickle.dump(data, f)
    logger.info("Cached NPB engine data to %s", engine_cache)
