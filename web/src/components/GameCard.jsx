  ...
  useEffect(() => {
    ...
    function update() {
      try {
        ...
        const lockMs = gameMs - 60 * 60 * 1000; // 1h before
        const now = Date.now();
...
export default function GameCard({ game: initialGame, teamNames, onPredictionSubmit }) {
    ...
    <div className="bg-slate-800/80 rounded-xl border border-slate-700 overflow-hidden hover:border-slate-600 transition-colors">
      ...
      >
        <div className="flex items-center justify-between">
          ...
          {/* Center: time + prediction bar or Get Prediction button */}
          <div className="flex-1 mx-4">
            {hasPred ? (
              <>
                <div className="flex justify-between text-xs mb-1">
                  <span className={favored === 'away' ? 'text-amber-400 font-bold' : 'text-slate-400'}>
                    {awayPct}%
                  </span>
                  <span className="text-slate-500">
                    {game.game_time} ET
                    {timeLeft && !isLocked && (
                      <span className="text-amber-500/70 text-xs ml-1 countdown-pulse">({timeLeft})</span>
                    )}
                    {isLocked && !submitted && (
                      <span className="text-red-400/70 text-xs ml-1">🔒</span>
                    )}
                  </span>
                  <span className={favored === 'home' ? 'text-amber-400 font-bold' : 'text-slate-400'}>
                    {homePct}%
                  </span>
                </div>
              ...
              <div className="text-center">
                ...
                <button
                  ...
                  disabled={predicting}
                  className="px-3 py-1.5 text-xs bg-amber-500/20 hover:bg-amber-500/30 text-amber-400 border border-amber-500/30 rounded-lg transition-colors disabled:opacity-50"
                >
                  {predicting ? (
      ...
      {/* Expanded: prediction form + details */}
      {expanded && (
        <div className="border-t border-slate-700 p-4 bg-slate-900/50">
          {/* Matchup details (only if prediction exists) */}
          {hasPred && (
            <>
              <div className="grid grid-cols-3 gap-4 text-center mb-4">
                <div>
                  ...
                  <div className="font-mono text-white">
                    {game.quick_away_score} - {game.quick_home_score}
                  </div>
                </div>
                <div>
                  <div className="text-xs text-slate-500">Avg Total</div>
                  <div className="font-mono text-white">
                    {game.mc_avg_total_runs.toFixed(1)} runs
                  </div>
                </div>
                <div>
          ...
          {!hasPred && (
            <div className="text-center py-4">
              <button
                onClick={handlePredict}
                disabled={predicting}
                className="px-6 py-2 bg-amber-500/20 hover:bg-amber-500/30 text-amber-400 border border-amber-500/30 rounded-lg transition-colors disabled:opacity-50"
              >
                {predicting ? (
                  <span className="flex items-center gap-2">
                    <span className="spinner inline-block w-4 h-4 border-2 border-amber-400/30 border-t-amber-400 rounded-full"></span>
