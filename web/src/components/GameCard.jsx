...
export default function GameCard({ game, teamNames, onPredictionSubmit }) {
  ...
  const awayPct = (game.final_away_win_pct * 100).toFixed(0);
  const homePct = (game.final_home_win_pct * 100).toFixed(0);
  const favored = game.final_away_win_pct > 0.5 ? 'away' : 'home';
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <div className="flex-1">
            <div className={`text-lg font-bold ${favored === 'away' ? 'text-amber-400' : 'text-white'}`}>
              {game.away_team_id}
            </div>
          ...
          <div className="flex-1 text-right">
            <div className={`text-lg font-bold ${favored === 'home' ? 'text-amber-400' : 'text-white'}`}>
              {game.home_team_id}
            </div>
